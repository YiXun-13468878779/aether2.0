...
export function DrawingSurface({ art, tool, color, width, opacity, onMark, onUndo, onRedo }: { art: Artwork; tool: Tool; color: string; width: number; opacity: number; onMark: (mark: Mark) => void; onUndo: () => void; onRedo: () => void }) {
  ...
  function finish(event: PointerEvent<HTMLCanvasElement>) { if (pointer.current !== event.pointerId || !pending.current) return; const mark = pending.current; pending.current = null; pointer.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); onMark(mark); }
  return <div className="art-paper" style={{ backgroundColor: art.paper }}>
    <canvas ref={canvasRef} width={WIDTH} height={HEIGHT} tabIndex={0} role="application" aria-label="自由创作画布，按 Command 或 Control 加 Z 撤销，加 Shift 重做" onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'z') { event.preventDefault(); if (event.shiftKey) onRedo(); else onUndo(); } }}
      onPointerDown={event => { if (!event.isPrimary || event.button !== 0 || !ready.current || pointer.current !== null) return; event.preventDefault(); event.currentTarget.focus({ preventScroll: true }); event.currentTarget.setPointerCapture(event.pointerId); pointer.current = event.pointerId; pending.current = { id: uid(), tool, color, width, opacity, points: [point(event)] }; preview(); }}
      onPointerMove={event => { const rect = event.currentTarget.getBoundingClientRect(); if (cursorRef.current) { cursorRef.current.style.left = event.clientX - rect.left + 'px'; cursorRef.current.style.top = event.clientY - rect.top + 'px'; cursorRef.current.style.width = Math.max(5, width * rect.width / WIDTH) + 'px'; cursorRef.current.style.height = Math.max(5, width * rect.width / WIDTH) + 'px'; cursorRef.current.style.opacity = '1'; } if (pointer.current !== event.pointerId || !pending.current) return; const next = point(event), last = pending.current.points.at(-1); if (!last || Math.hypot(next.x - last.x, next.y - last.y) > .8) { if (pending.current.points.length < 6000) pending.current.points.push(next); preview(); } }}
    ...
    <div ref={cursorRef} className="art-brush-cursor" />
  </div>;
}
